...
uint16_t TFT_eSPI::decodeUTF8(uint8_t *buf, uint16_t *index, uint16_t remaining, uint8_t *length)
{
  uint16_t c = buf[(*index)++];
  //
#ifdef DECODE_UTF8
...
uint32_t TFT_eSPI::getExceedOffset(void)
{
  _write_exceed_area = false;
  _write_len = 0;
  uint32_t len = _last_write_len;
  _last_write_len = 0;
  return len;
}
...
/***************************************************************************************
** Description:             Set a printf area
*************************************************************************************x*/
void TFT_eSPI::setTextArea(uint16_t x, uint16_t y, uint16_t w, uint16_t h)
{
  if((x > width()) || (y > height()))
  {
    return;
  ...
  _textarea_w = w;
  _textarea_h = h;
  this->setCursor(_textarea_x, _textarea_y);
}
/***************************************************************************************
** Function name:           drawChar
...
int16_t TFT_eSPI::drawString(const char *string, int32_t poX, int32_t poY, uint8_t font)
{
  ...
  if((gfxFont == NULL) && _is_freetype_loaded && _use_freetype_font)
  {
    ...
    if(textdatum == TL_DATUM || textdatum == TC_DATUM || textdatum == TR_DATUM)
    {
      base_y = poY + _current_ft_render->origin;
      fill_y = poY;
    }
    else if(textdatum == CL_DATUM || textdatum == CC_DATUM || textdatum == CR_DATUM)
    {
      base_y = poY + _current_ft_render->origin - (_current_ft_render->pixel_size >> 1);
      fill_y = poY - (_current_ft_render->pixel_size >> 1);
    }
    else if(textdatum == BL_DATUM || textdatum == BC_DATUM || textdatum == BR_DATUM)
    {
      base_y = poY + _current_ft_render->origin - _current_ft_render->pixel_size;
    ...
    if(is_fill_bg)
    {
      ...
      {
        fill_h = _current_ft_render->pixel_size + _fill_margin_bottom + _fill_margin_top;
      }
      else
      {
        auto_fill_margin = _current_ft_render->pixel_size >> 3;
        fill_h = _current_ft_render->pixel_size + (auto_fill_margin << 1);
      }
    }
    

    // Left/right alignment does not require pre-rendering
    if(textdatum == TL_DATUM || textdatum == CL_DATUM || textdatum == BL_DATUM)
    {
      if(is_fill_bg)
      {
        if(_fill_margin_active)
        {
          fillRect(poX - _fill_margin_left, fill_y - _fill_margin_top, _fill_margin_left, fill_h, textbgcolor);
        }
        else
        {
          fillRect(poX - auto_fill_margin, fill_y - auto_fill_margin, auto_fill_margin, fill_h, textbgcolor);
        }
      }
      while (n < len)
      {
        ...
        if(is_fill_bg)
        {
          if(_fill_margin_active)
          {
            fillRect(poX, fill_y - _fill_margin_top, _current_ft_render->advance, fill_h, textbgcolor);
          }
          else
          {
            fillRect(poX, fill_y - auto_fill_margin, _current_ft_render->advance, fill_h, textbgcolor);
          }
        }
        drawFreetypeBitmap(poX + _current_ft_render->bitmap_left, base_y - _current_ft_render->bitmap_top, _current_ft_render->bitmap_width, _current_ft_render->bitmap_height, _current_ft_render->bitmap);
      ...
      if(is_fill_bg)
      {
        if(_fill_margin_active)
        {
          fillRect(poX, fill_y - _fill_margin_top, _fill_margin_right, fill_h, textbgcolor);
        }
        else
        {
          fillRect(poX, fill_y - auto_fill_margin, auto_fill_margin, fill_h, textbgcolor);
        }
      }
      return sumX;
    }
    
    if(textdatum == TR_DATUM || textdatum == CR_DATUM || textdatum == BR_DATUM)
    {
      uint16_t uniCodes[len];
      uint16_t unicode_num = 0;
      while (n < len)
      {
        uniCodes[unicode_num] = decodeUTF8((uint8_t*)string, &n, len - n);
        unicode_num++;
      }
    ...
    // Center alignment requires pre-rendering, which needs to temporarily occupy some memory
    uint16_t cwidth = 0;
    uint16_t unicode_num = 0;
    n = 0;
    while (n < len)
    ...
    while (n < len)
    {
      ...
      memcpy(tempcache[idx].bitmap, _current_ft_render->bitmap, bitmap_size);
    ...
    if(is_fill_bg)
    {
      if(_fill_margin_active)
      {
        fillRect(poX - _fill_margin_left, fill_y - _fill_margin_top, cwidth + _fill_margin_left + _fill_margin_right, fill_h, textbgcolor);
      }
      else
      {
        fillRect(poX - auto_fill_margin, fill_y - auto_fill_margin, cwidth + (auto_fill_margin << 1), fill_h, textbgcolor);
      }
    }
    ...
    for(int i = 0; i < unicode_num; i++)
    {
      if(tempcache[i].bitmap != nullptr)
      {
        drawFreetypeBitmap(poX + tempcache[i].bitmap_left, base_y - tempcache[i].bitmap_top, tempcache[i].bitmap_width, tempcache[i].bitmap_height, tempcache[i].bitmap);
...
void TFT_eSPI::setTextFillMargin(uint16_t left, uint16_t right, int16_t top, int16_t bottom)
{
...
void TFT_eSPI::drawFreetypeBitmap(int32_t cx, int32_t cy, uint16_t bw, uint16_t bh, uint8_t *bitmap)
{
...
void TFT_eSPI::drawFreetypeGlyph(uint16_t code)
{
  ...
  if (code == '\n') {
    ...
    if (textwrapY && cursor_y + _current_ft_render->pixel_size >= _textarea_y + _textarea_h)
    {
  ...
  if (textwrapX && (this->cursor_x + _current_ft_render->advance > (_textarea_x + _textarea_w)))
  {
    this->cursor_y += _current_ft_render->pixel_size + _ft_linespace;
    this->cursor_x = _textarea_x;
  }
  if (textwrapY && (this->cursor_y + _current_ft_render->pixel_size >= (_textarea_y + _textarea_h)))
  {
    this->cursor_y = _textarea_y;
  }
  if (cursor_x == _textarea_x) cursor_x -= _current_ft_render->bitmap_left;

  int16_t cy = cursor_y - _current_ft_render->bitmap_top + _current_ft_render->origin;
  int16_t cx = cursor_x + _current_ft_render->bitmap_left;

  drawFreetypeBitmap(cx, cy, _current_ft_render->bitmap_width, _current_ft_render->bitmap_height, _current_ft_render->bitmap);
...
** Description:             Page through all characters in font, td ms between screens
*************************************************************************************x*/
#if(defined USE_M5_FONT_CREATOR && defined LOAD_GFXFF)
